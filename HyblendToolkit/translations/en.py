...
TRANSLATIONS = {
    ...
    "importer.prop.blockymodel_texture_mode": (
        ...
        "folder as the model, or a '<ModelName>_Textures' subfolder). As a "
        "last resort it also looks in any folder with 'texture' in its "
        "name one level above the model. Variants are all loaded and "
        "stacked in the material. 'Manual' lets you point to a specific "
        "file instead, ignoring auto-detection entirely"
    ),
    "importer.prop.blockymodel_texture_mode_item_auto": (
        "Auto-detect the texture PNG next to the model (same convention "
        "as the official Hytale plugin), or in a 'texture' folder one level above"
    ),
    "importer.prop.texture_mode_item_manual": (
        "Pick the texture yourself -- a PNG file, or a folder (its PNGs and "
        "subfolders are searched with the same name filter as Automatic)"
    ),
    "importer.prop.blockymodel_texture_filepath": (
        "The model's texture PNG -- or a FOLDER: then every PNG in it and in "
        "its subfolders whose name starts with the model name (or the model "
        "name without its last '_part') is loaded, same filter as Automatic; "
        "the first one is connected, the others are stacked in the material. "
        "The .blockymodel file only stores per-face pixel offsets, not the "
        "texture itself or its canvas size -- pointing this at the real file "
        "gives exact UVs using its actual dimensions (takes priority over "
        "'Set Atlas Size Manually' above). Only used when 'Texture Mode' "
    ...
    "importer.model_format": "Model Type",
    "importer.prop.blockymodel_model_format": (
        "Character (64 units per block) or Prop (32 per block) -- decides "
        "the scale. Auto-Detect reads the file's \"format\" field (written "
        "by the official Blockbench plugin); without it, an attachment "
        "inherits the target Armature's type, and a file inside a "
        "\"Blocks\" folder is treated as Prop"
    ),
    "importer.prop.bbmodel_model_format": (
        "Character (64 units per block) or Prop (32 per block) -- decides "
        "the scale. Auto-Detect reads the project format saved in the "
        ".bbmodel (Hytale Character / Hytale Prop); any other format is "
        "treated as Character"
    ),
    "importer.prop.model_format_item_auto": "Detect from the file",
    "importer.prop.model_format_item_character": (
        "Characters, creatures, cosmetics, tools, weapons -- 64 units per block"
    ),
    "importer.prop.model_format_item_prop": (
        "Props, blocks, furniture -- 32 units per block"
    ),
    ...
    "exporter.prop.blockyanim_unit_scale": (
        ...
        "default"
        " -- Value for Character models (64 units per block). Prop models (32 per block) automatically use double this, so both end up at the same size in Blender"
    ),
    ...
    # ---------------------------------------------------------------
    # v0.15 -- .blockymodel export (Model). Position/orientation come
    # from the current rest pose; shape/UV come from what the importer
    # captured at import time (see common.BONE_SHAPE_JSON_PROP).
    # ---------------------------------------------------------------
    "exporter.prop.blockymodel_unit_scale": (
        "MUST match the exact value used when this character was "
        "imported (Hytale Blockymodel Importer) -- if they don't "
        "match, every position in the exported file will be wrong by "
        "a consistent scale factor. Same field as the .blockyanim "
        "exporter"
        " -- Value for Character models (64 units per block). Prop models (32 per block) automatically use double this, so both end up at the same size in Blender"
    ),
    "exporter.prop.blockymodel_output_decimal_places": (
        "How many digits after the decimal point to keep for every "
        "number in the file. Purely cosmetic, doesn't drop any data -- "
        "just keeps the file from being full of numbers like "
        "0.30000000000000004"
    ),
    ...
    # ---------------------------------------------------------------
    # v0.15 -- FBX export (Model + Animation). Reuses the same Export
    # Bone Collection concept as .blockyanim -- see
    # exporter.resolve_exportable_bone_names.
    # ---------------------------------------------------------------
    "exporter.prop.fbx_include_meshes": (
        "Include this character's reference meshes (Main + optionally "
        "Mesh Attachments collections) in the .fbx, alongside the "
        "skeleton. OFF exports the Armature only -- no geometry at all"
    ),
    "exporter.prop.fbx_include_attachments": (
        "Only matters if 'Include Reference Meshes' is ON. Also "
        "includes meshes from this Armature's Mesh Attachments "
        "collection (anything imported via 'Attach to Existing'), not "
        "just the Main collection"
    ),
    "exporter.prop.fbx_match_game_units": (
        "OFF (default): exports at Blender's own scale (1 Blender unit "
        "= 1 Blender unit) -- what most other software expects when "
        "importing an .fbx. ON: multiplies by 1 / 'Blender Units per "
        "Game Unit' so the exported file matches Hytale's raw in-game "
        "units instead -- only useful if whatever reads this .fbx "
        "specifically needs that"
    ),
    "exporter.prop.fbx_unit_scale": (
        "Only used when 'Scale to Game Units' is ON. MUST match the "
        "exact value used when this character was imported (Hytale "
        "Blockymodel Importer), same as the equivalent field in the "
        ".blockyanim exporter"
        " -- Value for Character models (64 units per block). Prop models (32 per block) automatically use double this, so both end up at the same size in Blender"
    ),
    "exporter.prop.fbx_apply_transform": (
        "OFF (default): the Armature's own object rotation/scale (e.g. "
        "the 90 degree 'Orient to Z-up' rotation some imports add) is "
        "kept as a transform on the FBX node, the way most game "
        "engines/DCCs expect. ON: bakes that transform directly into "
        "the exported vertex/bone data instead, leaving the node's own "
        "transform at identity -- turn this on only if whatever reads "
        "the file doesn't handle a non-identity root transform well"
    ),
    "exporter.prop.fbx_add_leaf_bones": (
        "Blender's own FBX exporter option: adds an extra tip bone at "
        "the end of every bone chain with no children, matching how "
        "3ds Max/older pipelines expect a bone's length to be "
        "represented. Most modern engines don't need this -- leave OFF "
        "unless you know your target pipeline wants it"
    ),
    "exporter.prop.fbx_embed_textures": (
        "Only matters if 'Include Reference Meshes' is ON and those "
        "meshes have a material with an image texture. OFF (default): "
        "the .fbx only references the texture file's path -- ON: "
        "copies the image data directly inside the .fbx, so it doesn't "
        "depend on that external file anymore (bigger file, but fully "
        "self-contained)"
    ),
    ...
    "panel.btn_more_import_options": "More Options",
    ...
    "panel.btn_export_model": "Export Model (.blockymodel)",
    ...
    "panel.more_exports": "More Exports",
    "panel.hint_more_exports": "Other formats, separate from the round-trip .blockyanim workflow above",
    "panel.btn_export_fbx_model": "Export Model (.fbx)",
    "panel.btn_export_fbx_anim": "Export Animations (.fbx)",
    ...
    # v0.16 -- "Main Head", exclusivo de HEAD (ver
    # HytaleIKChainItem.head_is_main em rigger/bone_settings.py) --
    # desenhado ACIMA de "Head Free/Lock" de propósito (pedido explícito
    # do usuário): decide qual entrada HEAD é "a" cabeça principal
    # quando existe mais de uma (criatura com várias cabeças).
    "panel.field_head_is_main": "Main Head",
    ...
    "panel.field_root_count": "Root Bones Amount",
    "panel.field_root_origin": "Origin",
    "panel.field_root": "Root",
    "panel.hint_root_order": "Each Root controls the one above it (Root 2 moves Origin)",
    "panel.hint_root_new_bone": "+ = create the bone (Blender only, never exported)",
    # Parte 4 -- Rename Bones.
    "panel.field_rename_enabled": "Rename Bones (Hytale names)",
    "panel.hint_rename": "Checked fields get the Hytale name (L-Arm, Head...). Export can still use the original names",
    "panel.btn_rename_bones": "Rename Bones",
    "panel.rename_nothing": "Nothing to rename -- the checked bones already use the standard names",
    "panel.rename_preview_title": "{count} bone(s) will be renamed:",
    "panel.rename_preview_more": "...and {count} more",
    "panel.rename_preview_title_ctrl": "{count} bone(s) will get Hytale-named controls (original bones keep their names):",
    "panel.hint_rename_only_ctrl": "Only the control bones (_CTRL, _IK, Pole) get Hytale names. ORG/MCH keep theirs",
    "panel.rename_will_clear": "The generated rig will be removed first -- run Create Rig afterwards",
    ...
    "panel.hint_shape_edit_active_elsewhere": "'{name}' is still in Shape Edit Mode.",
    "panel.btn_shape_edit_mode_reselect": "Select It",
    ...
    "panel.hint_vertex_edit_orphaned": "The widget mesh being edited was deleted -- finish to clear this state.",
    ...
    "rigger.tooltip.shape_edit_mode_reselect": (
        "Select and activate the Armature that is still in Shape Edit Mode, so you can use "
        "'Finish Shape Edit Mode' on it"
    ),
    ...
    "rigger.prop.ik_chain_chain_type_item_root": (
        "Marks the root bone(s): Root 1 is the main Origin, extra roots stack above it. Can create "
        "missing root bones (Blender only, never exported)"
    ),
    ...
    "rigger.prop.ik_chain_head_is_main": (
        "Marks this as THE main Head -- the one used for PROPERTIES (FK/IK switch), Head Follow, the "
        "Main/Head bone collection and the dedicated Head widget/color, when there's more than one HEAD "
        "entry in the list (e.g. a multi-head creature). Behaves like a radio button: checking it here "
        "unchecks any other HEAD entry automatically. The first HEAD entry you add starts checked; any "
        "HEAD entry added afterward starts unchecked"
    ),
    ...
    "rigger.prop.armature_rename_mode": "What 'Rename Bones' renames",
    "rigger.prop.armature_rename_mode_item_all": (
        "Rename the original bones -- every generated bone follows (L-Arm, L-Arm_CTRL, L-Arm_MCH...). The "
        "old names are remembered: 'Use Original Names' in the .blockymodel/.blockyanim export writes them"
    ),
    "rigger.prop.armature_rename_mode_item_only_ctrl": (
        "Keep the original (ORG) and MCH bones as they are; only the control bones (_CTRL, _IK, _Pole_CTRL, "
        "_Pole_Line) get the Hytale names. Nothing changes in the model or in FBX export"
    ),
    "rigger.prop.armature_rename_enabled": (
        "Shows a checkbox next to each bone picker in Bone Settings and the 'Rename Bones' button, which "
        "renames the checked bones to the Hytale standard names (L-Arm, L-Forearm, Head, Pelvis...) so "
        "L/R mirroring works. The old names are remembered (revert button, 'Use Original Names' on export)"
    ),
    "rigger.prop.ik_chain_rename_field": (
        "Rename this bone to the Hytale standard name when 'Rename Bones' runs. Unchecked = keeps its "
        "original name"
    ),
    "rigger.prop.ik_chain_rename_middle": (
        "Rename the Pole Reference and every bone between Root and Tip (Forearm, Forearm2... / Calf...) "
        "when 'Rename Bones' runs. Unchecked = they keep their original names"
    ),
    "rigger.tooltip.rename_bones": (
        "Rename the checked Bone Settings bones to the Hytale standard names. Removes the generated rig "
        "first if there is one (run Create Rig afterwards). The old names are remembered for the revert "
        "button and for 'Use Original Names' on export. Undo with Ctrl+Z"
    ),
    "rigger.tooltip.revert_bone_names": (
        "Revert 'Rename Bones': every renamed bone gets its original name back and the Hytale control names "
        "(Only CTRL) are cleared -- both modes at once. Removes the generated rig first if there is one (run "
        "Create Rig afterwards). Undo with Ctrl+Z"
    ),
    "panel.revert_nothing": "Nothing to revert -- no bone was renamed",
    "panel.revert_preview_title": "{count} name(s) will go back to the original:",
    "exporter.prop.use_original_names": (
        "Bones renamed with 'Rename Bones' (All Bones) are written with the name they had before the rename "
        "(the game's name) instead of the new one. OFF (default): the new names are written. Only available "
        "when some bone was renamed"
    ),
    "rigger.prop.ik_chain_root_count": (
        "How many root bones (1-{max}). Root 1 is the main Origin; Root 2 becomes the parent of Root 1, "
        "Root 3 the parent of Root 2, and so on -- everything already under the Origin stays there"
    ),
    "rigger.prop.ik_chain_root_slot_bone": (
        "ORIGINAL bone name (not '_CTRL') used as this root. With 'New Bone' on, this is the name of the "
        "bone to create (empty = 'Origin' for Root 1, 'Root<N>' for the others)"
    ),
    "rigger.prop.ik_chain_root_create": (
        "Create this root bone instead of using one from the model. The created bone exists only in "
        "Blender as a control -- it is never exported to .blockymodel/.blockyanim"
    ),
    ...
    "exporter.tooltip.blockymodel": (
        "Export the selected/active Armature's rest pose (bind pose) to Hytale's "
        ".blockymodel format -- position/orientation come from the current bone "
        "transforms, shape/UV data comes from what was captured when the character was "
        "imported"
    ),
    "exporter.tooltip.fbx_model": (
        "Export the selected/active Armature's rest pose (bind pose) -- and, optionally, "
        "its reference meshes -- to a single .fbx file. No animation is included; use "
        "'Hytale Animations (.fbx)' for that"
    ),
    "exporter.tooltip.fbx_anim": (
        "Batch-export one or more Actions of the selected/active Armature to .fbx -- one "
        "file per Action (rig + baked animation, and optionally the reference meshes), "
    ...
    "importer.tooltip.bedrock_model": (
        "Import a Minecraft Bedrock model (.json -- Bedrock add-ons and "
        "mods) converted to the Hytale standard: "
        "scale, texture density and inflate"
    ),
    "importer.bedrock_section_conversion": "Conversion",
    "importer.bedrock_geometry_identifier": "Geometry",
    "importer.bedrock_inflate_to_stretch": "Inflate to Stretch",
    "importer.bedrock_drop_locator_bones": "Remove Locator Bones",
    "importer.prop.bedrock_model_format": (
        "Minecraft doesn't say whether a model is a creature or a block, "
        "so choose it here. Minecraft uses 16 units per block; Hytale "
        "uses 64 (Character) or 32 (Prop). The model keeps its size in "
        "the world -- geometry, UVs and texture are scaled up to match"
    ),
    "importer.prop.bedrock_model_format_item_character": (
        "Creatures, mobs, characters, wearables -- geometry and texture x4 (16 -> 64 per block)"
    ),
    "importer.prop.bedrock_model_format_item_prop": (
        "Blocks, furniture, props -- geometry and texture x2 (16 -> 32 per block)"
    ),
    "importer.prop.bedrock_geometry_identifier": (
        "Which geometry to import when the file has more than one (e.g. "
        "geometry.my_model or just my_model). Empty = the first one"
    ),
    "importer.prop.bedrock_armature_name": (
        "Name for the new Armature and its collection. Empty = the "
        "geometry name (without 'geometry.')"
    ),
    "importer.prop.bedrock_inflate_to_stretch": (
        "Hytale has no 'inflate'. Convert it to 'stretch' around the "
        "cube's center, which gives the same result without touching "
        "the UVs. Flat cubes (size 0) stay flat"
    ),
    "importer.prop.bedrock_drop_locator_bones": (
        "Skip bones that only hold locators (effect/item attachment "
        "points used by Minecraft mods, e.g. locator_mouth) and have no "
        "cubes"
    ),
    "importer.prop.bedrock_texture_mode": (
        "'Automatic' finds the texture PNG the same way the .blockymodel "
        "importer does: a PNG in the model's folder whose name starts with "
        "the model name (file name without '.geo.json'), 'Texture.png', or "
        "a '<ModelName>_Textures' subfolder -- and, as a last resort, a "
        "folder with 'texture' in its name one level above the model. "
        "Variants are all loaded and stacked in the material. 'Manual' "
        "uses only the file you pick"
    ),
    "importer.prop.bedrock_texture_mode_item_auto": (
        "Auto-detect the texture PNG (same search as the .blockymodel importer)"
    ),
    "importer.prop.bedrock_texture_filepath": (
        "The model's PNG (Texture Mode = Manual) -- or a FOLDER: then every "
        "PNG in it and in its subfolders whose name starts with the model "
        "name (or the model name without its last '_part') is loaded, same "
        "filter as Automatic; the first one is connected, the others are "
        "stacked in the material. Bedrock models don't store the texture. "
        "The imported copy is enlarged (pixel-perfect) to Hytale's density; "
        "your original files aren't changed"
    ),
    ...
    "importer.prop.unit_scale": (
        "Blender units per game unit, for Character models (64 units per "
        "block) -- the default 1/64 makes 1 block = 1 meter. Prop models "
        "(32 per block) automatically use double this, so both end up at "
        "the same size in Blender. The exporters use the same value to "
        "convert back, so leave it at the default unless you're sure"
    ),
    "importer.prop.generate_reference_boxes": (
        "Creates a mesh for each cube in the model, parented to the "
        "Armature and skinned (100% weight) to its bone via a Vertex Group "
        "+ Armature modifier"
    ),
    "importer.prop.generate_uvs": (
        "Generates UV coordinates for the reference meshes from each "
        "face's pixel rectangle in the model"
    ),
    "importer.prop.create_material": (
        "Creates one material with the texture found by 'Texture Mode', "
        "wired into Base Color/Alpha through the generated UVs"
    ),
}
