...
from ..common import SUFFIX_CTRL, SUFFIX_IK, SUFFIX_MCH  # noqa: F401 (SUFFIX_MCH: reexport pro rigger/__init__.py)
...
# Rename "Only CTRL": apelido gravado no bone ORG -- os bones de controle
# (_CTRL, _IK, _Pole_CTRL, _Pole_Line) nascem com esse nome-base em vez do
# nome do ORG ("leg_front_right" + apelido "R-Arm" -> "R-Arm_CTRL"). ORG,
# _MCH e as pontes _MCH_Transfer/_MCH_IK_Transfer continuam com o nome do
# ORG. Ver control_name()/helpers.py e rigger/rename.py.
PROP_CONTROL_NAME = "hytale_control_name"
# Gravado em todo bone gerado a partir de um ORG: de qual ORG ele nasceu.
# É o caminho de volta controle -> ORG (tirar o sufixo do nome não funciona
# quando o controle usa um apelido). Rig gerado antes disto não tem a marca
# -- source_org_of() cai pro jeito antigo (tirar o sufixo).
PROP_SOURCE_ORG = "hytale_source_org"
...
# LEGADO -- só vale pra Armature SEM Bone Settings correspondente. Hoje
# os parents forçados saem do Bone Settings (ver
# _resolve_ctrl_parent_overrides em generate.py): o _CTRL do Pelvis da
# SPINE vai pro root.pelvis_CTRL, o do Spine 1 pro root.master_CTRL, e o
# _CTRL do Root Bone de cada LEG pro mesmo "Root Parent" que o IK dela
# usa. Este dict só entra nas partes sem entrada: sem SPINE -> as linhas
# Pelvis/Belly; sem nenhuma LEG -> as linhas L-/R-Thigh.
#
...
# Os bones Spine 1..N da entrada SPINE (acima do Pelvis) seguem
# parcialmente o root.spine_CTRL via um constraint de Copy Transforms
# (Local Space), com influência fixa por posição na coluna: Spine 1 =
# 0.5, Spine 2 = 0.63, e daí em diante sobe no mesmo passo (0.13). O
# Pelvis nunca recebe Spine Follow. Ver resolve_spine_segment_org_names
# (helpers.py) e _build_spine_follow (generate.py).
SPINE_FOLLOW_TARGET = BONE_ROOT_SPINE
SPINE_FOLLOW_INFLUENCES = (0.5, 0.63, 0.76, 0.89)
# Legado -- só usado quando o Armature NÃO tem entrada SPINE no Bone
# Settings (nomes do Player).
SPINE_FOLLOW_BONES = {
    "Belly_CTRL": 0.5,
    "Chest_CTRL": 0.63,
}
...
# Entrada ROOT do Bone Settings: quantos slots de root (Root 1 = Origin
# principal; Root 2 vira pai do Root 1, Root 3 pai do Root 2...). Campos
# root_bone_1..N / root_create_1..N gerados num loop com este número.
ROOT_MAX_COUNT = 4
# Bone de root CRIADO pelo rigger ("New Bone"): comprimento cresce por
# slot (ORIGIN_FALLBACK_LENGTH * (1 + passo * (slot - 1))) -- o custom
# shape escala com o comprimento, então roots criados no mesmo ponto
# ficam concêntricos (cada um maior que o de dentro) em vez de
# empilhados um em cima do outro, impossíveis de selecionar.
ROOT_CREATED_LENGTH_STEP = 0.5
...
# Cor de bone em 3 níveis (ver _build_bone_colors), do mais forte pro
# mais fraco:
#   1. BONE_COLOR_FIXED -- bones utilitários que o próprio rigger cria
#      com nome FIXO (root.*, Origin_CTRL, PROPERTIES). Não vêm de
#      nenhuma entrada do Bone Settings, então nada pode competir com eles.
#   2. Bone Settings -- o que o usuário MARCOU (Arm/Leg + lado, Head,
#      Neck, Spine, Chain, Attachments) vence qualquer nome. Ver
#      _bone_settings_color_map em generate.py.
#   3. BONE_COLOR_NAME_FALLBACK + dica de attachment + token de lado no
#      nome (L-/R-, _left/_right, .L/.R...) -- só pra bone que não está
#      em nenhuma entrada do Bone Settings (padrão Hytale/Player sem
#      Bone Settings configurado, olhos/mandíbula/chifres etc.).
BONE_COLOR_FIXED = {
...
# Nomes do padrão Hytale/Player -- só FALLBACK (nível 3). Um bone com
# esses nomes que esteja marcado no Bone Settings com outro papel segue
# o Bone Settings.
BONE_COLOR_NAME_FALLBACK = {
    ...
    **{name: BONE_COLOR_SPINE for name in SPINE_COLLECTION_BONES},
    "Neck" + SUFFIX_CTRL: BONE_COLOR_SPINE,
}
# Mantido por compatibilidade (união dos dois acima) -- o gerador não
# usa mais este dict direto.
