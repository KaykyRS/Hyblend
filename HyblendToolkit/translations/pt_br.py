...
TRANSLATIONS = {
    ...
    "importer.prop.flat_mesh_collections": (
    ...
    "importer.prop.blockymodel_texture_mode": (
        "'Automático' procura o PNG da textura no disco usando a mesma "
        "convenção do plugin oficial do Hytale pro Blockbench (mesma pasta "
        "do modelo, ou uma subpasta '<NomeDoModelo>_Textures'). Como último "
        "recurso, olha também qualquer pasta com 'texture' no nome um nível "
        "acima do modelo. Variantes são todas carregadas e empilhadas no "
        "material. 'Manual' deixa você apontar um arquivo específico, "
        "ignorando a detecção automática"
    ),
    "importer.prop.blockymodel_texture_mode_item_auto": (
        "Detecta o PNG ao lado do modelo (mesma convenção do plugin oficial "
        "do Hytale), ou numa pasta 'texture' um nível acima"
    ),
    "importer.prop.texture_mode_item_manual": (
        "Escolha a textura você mesmo -- um arquivo PNG, ou uma pasta (os "
        "PNGs dela e das subpastas são buscados com o mesmo filtro de nome "
        "do Automático)"
    ),
    "importer.prop.blockymodel_texture_filepath": (
        "O PNG de textura do modelo -- ou uma PASTA: aí todo PNG dela e das "
        "subpastas cujo nome começa com o nome do modelo (ou com o nome sem "
        "o último '_pedaço') é carregado, mesmo filtro do Automático; o "
        "primeiro fica ligado, os outros empilhados no material. O "
        ".blockymodel só guarda offsets de pixel por face, não a textura nem "
        "o tamanho do canvas -- apontar pro arquivo real dá UVs exatas usando "
        "as dimensões dele (tem prioridade sobre 'Set Atlas Size Manually' "
        "acima). Só usado quando 'Texture Mode' acima está em 'Manual'"
    ),
    "importer.prop.orient_z_up": (
    ...
    "importer.model_format": "Tipo de Modelo",
    "importer.prop.blockymodel_model_format": (
        "Character (64 unidades por bloco) ou Prop (32 por bloco) -- define "
        "a escala. Auto-Detect lê o campo \"format\" do arquivo (gravado "
        "pelo plugin oficial do Blockbench); sem ele, um attachment herda "
        "o tipo da Armature alvo, e um arquivo dentro de uma pasta "
        "\"Blocks\" é tratado como Prop"
    ),
    "importer.prop.bbmodel_model_format": (
        "Character (64 unidades por bloco) ou Prop (32 por bloco) -- define "
        "a escala. Auto-Detect lê o formato de projeto salvo no .bbmodel "
        "(Hytale Character / Hytale Prop); qualquer outro formato é "
        "tratado como Character"
    ),
    "importer.prop.model_format_item_auto": "Detectar pelo arquivo",
    "importer.prop.model_format_item_character": (
        "Personagens, criaturas, cosméticos, ferramentas, armas -- 64 unidades por bloco"
    ),
    "importer.prop.model_format_item_prop": (
        "Props, blocos, móveis -- 32 unidades por bloco"
    ),
    ...
    "exporter.prop.blockyanim_unit_scale": (
        ...
        " -- Valor pra modelos Character (64 unidades por bloco). Modelos Prop (32 por bloco) usam o dobro automaticamente, pra os dois ficarem no mesmo tamanho no Blender"
    ),
    ...
    # ---------------------------------------------------------------
    # v0.15 -- Export de .blockymodel (Model). Posição/orientação vêm
    # da pose de repouso atual; shape/UV vêm do que o importer capturou
    # na hora do import (ver common.BONE_SHAPE_JSON_PROP).
    # ---------------------------------------------------------------
    "exporter.prop.blockymodel_unit_scale": (
        "TEM que bater com o valor exato usado quando este personagem "
        "foi importado (Hytale Blockymodel Importer) -- se não "
        "baterem, toda posição no arquivo exportado sai errada por um "
        "fator de escala consistente. Mesmo campo do exportador de "
        ".blockyanim"
        " -- Valor pra modelos Character (64 unidades por bloco). Modelos Prop (32 por bloco) usam o dobro automaticamente, pra os dois ficarem no mesmo tamanho no Blender"
    ),
    "exporter.prop.blockymodel_output_decimal_places": (
        "Quantos dígitos depois da vírgula manter em todo número do "
        "arquivo. Puramente cosmético, não descarta nenhum dado -- só "
        "evita que o arquivo fique cheio de números tipo "
        "0.30000000000000004"
    ),
    "exporter.prop.blockymodel_pretty_print_json": (
        "DESLIGADO (padrão): escreve o arquivo numa linha compacta só "
        "-- menor, e normalmente ninguém precisa ler isso à mão. "
        "LIGADO: escreve bem indentado em várias linhas em vez disso, "
        "só pra um humano conseguir abrir e ler/comparar (mais ou "
        "menos dobra o tamanho do arquivo; o jogo e o Blockbench leem "
        "os dois formatos igualmente)"
    ),

    # ---------------------------------------------------------------
    # v0.15 -- Export FBX (Model + Animation). Reaproveita o mesmo
    # conceito de Export Bone Collection do .blockyanim -- ver
    # exporter.resolve_exportable_bone_names.
    # ---------------------------------------------------------------
    "exporter.prop.fbx_include_meshes": (
        "Inclui as malhas de referência deste personagem (collections "
        "Main + opcionalmente Mesh Attachments) no .fbx, junto com o "
        "esqueleto. DESLIGADO exporta só o Armature -- sem geometria "
        "nenhuma"
    ),
    "exporter.prop.fbx_include_attachments": (
        "Só importa se 'Include Reference Meshes' estiver LIGADO. "
        "Também inclui malhas da collection Mesh Attachments deste "
        "Armature (qualquer coisa importada via 'Attach to Existing'), "
        "não só a collection Main"
    ),
    "exporter.prop.fbx_match_game_units": (
        "DESLIGADO (padrão): exporta na escala do próprio Blender (1 "
        "unidade do Blender = 1 unidade do Blender) -- o que a maioria "
        "dos outros programas espera ao importar um .fbx. LIGADO: "
        "multiplica por 1 / 'Blender Units per Game Unit', fazendo o "
        "arquivo exportado bater com as unidades cruas do jogo -- só "
        "útil se o que for ler esse .fbx precisar especificamente disso"
    ),
    "exporter.prop.fbx_unit_scale": (
        "Só usado quando 'Scale to Game Units' está LIGADO. TEM que "
        "bater com o valor exato usado quando este personagem foi "
        "importado (Hytale Blockymodel Importer), igual ao campo "
        "equivalente no exportador de .blockyanim"
        " -- Valor pra modelos Character (64 unidades por bloco). Modelos Prop (32 por bloco) usam o dobro automaticamente, pra os dois ficarem no mesmo tamanho no Blender"
    ),
    "exporter.prop.fbx_apply_transform": (
        "DESLIGADO (padrão): a própria rotação/escala do Object "
        "Armature (ex.: a rotação de 90 graus que 'Orient to Z-up' "
        "adiciona em alguns imports) fica guardada como um transform no "
        "node do FBX, do jeito que a maioria dos engines/DCCs espera. "
        "LIGADO: assa esse transform direto nos dados de vértice/bone "
        "exportados, deixando o transform do próprio node em identidade "
        "-- só ligue se o que for ler o arquivo não lidar bem com um "
        "transform de raiz que não seja identidade"
    ),
    "exporter.prop.fbx_add_leaf_bones": (
        "Opção do próprio exportador FBX do Blender: adiciona um bone "
        "de ponta extra no fim de toda cadeia de bones sem filhos, "
        "igual ao 3ds Max/pipelines mais antigos esperam que o "
        "comprimento de um bone seja representado. A maioria dos "
        "engines modernos não precisa disso -- deixe DESLIGADO a não "
        "ser que você saiba que seu pipeline de destino quer isso"
    ),
    "exporter.prop.fbx_embed_textures": (
        "Só importa se 'Include Reference Meshes' estiver LIGADO e "
        "essas malhas tiverem um material com textura de imagem. "
        "DESLIGADO (padrão): o .fbx só referencia o caminho do arquivo "
        "de textura -- LIGADO: copia os dados da imagem direto dentro "
        "do .fbx, então ele deixa de depender desse arquivo externo "
        "(arquivo maior, mas totalmente autocontido)"
    ),
    ...
    "panel.btn_more_import_options": "Mais Opções",
    "panel.btn_import_bedrock": "Bedrock (.json)",
    ...
    "panel.btn_export_model": "Exportar Modelo (.blockymodel)",
    ...
    "panel.more_exports": "Mais Exports",
    "panel.hint_more_exports": "Outros formatos, separados do fluxo round-trip .blockyanim acima",
    "panel.btn_export_fbx_model": "Exportar Modelo (.fbx)",
    "panel.btn_export_fbx_anim": "Exportar Animações (.fbx)",
    ...
    "panel.field_head_is_main": "Cabeça Principal",
    ...
    # Parte 3 -- entrada ROOT do Bone Settings.
    "panel.field_root_count": "Quantidade de Roots",
    "panel.field_root_origin": "Origin",
    "panel.field_root": "Root",
    "panel.hint_root_order": "Cada Root controla o de cima (Root 2 move o Origin)",
    "panel.hint_root_new_bone": "+ = cria o bone (só no Blender, nunca exportado)",
    # Parte 4 -- Rename Bones.
    "panel.field_rename_enabled": "Renomear Bones (nomes do Hytale)",
    "panel.hint_rename": "Campos marcados ganham o nome do Hytale (L-Arm, Head...). O export ainda pode usar os nomes originais",
    "panel.btn_rename_bones": "Renomear Bones",
    "panel.rename_nothing": "Nada pra renomear -- os bones marcados já usam os nomes padrão",
    "panel.rename_preview_title": "{count} bone(s) vão ser renomeados:",
    "panel.rename_preview_more": "...e mais {count}",
    "panel.rename_preview_title_ctrl": "{count} bone(s) vão ter controles com nome do Hytale (os originais mantêm o nome):",
    "panel.hint_rename_only_ctrl": "Só os bones de controle (_CTRL, _IK, Pole) ganham nome do Hytale. ORG/MCH mantêm o deles",
    "panel.rename_will_clear": "O rig gerado vai ser removido antes -- rode o Create Rig depois",
    ...
    # v0.17 -- dica + botão de "voltar" quando o objeto ativo mudou
    # enquanto uma Armature ainda está em Shape Edit Mode (ver
    # find_armature_stuck_in_shape_edit_mode em rigger/shape_edit.py).
    "panel.hint_shape_edit_active_elsewhere": "'{name}' ainda está em Shape Edit Mode.",
    "panel.btn_shape_edit_mode_reselect": "Selecionar",
    # v0.17 -- caminho de recuperação quando o widget foi deletado no
    # meio do Vertex Edit Mode (ver RIG_OT_hytale_shape_vertex_edit_mode_finish).
    "panel.hint_vertex_edit_orphaned": "A malha do widget sendo editada foi apagada -- finalize pra limpar esse estado.",
    ...
    "rigger.tooltip.shape_edit_mode_reselect": (
        "Seleciona e ativa a Armature que ainda está em Shape Edit Mode, pra dar pra usar "
        "'Finish Shape Edit Mode' nela"
    ),
    ...
    "rigger.prop.ik_chain_chain_type_item_root": (
        "Marca o(s) bone(s) de root: o Root 1 é o Origin principal, roots extras ficam acima dele. "
        "Pode criar os bones de root que faltarem (só no Blender, nunca exportados)"
    ),
    ...
    "rigger.prop.ik_chain_head_is_main": (
        "Marca esta como A cabeça principal -- a usada pro PROPERTIES (switch FK/IK), Head Follow, "
        "a bone collection Main/Head e o widget/cor dedicados de Head, quando existe mais de uma "
        "entrada HEAD na lista (ex.: uma criatura com várias cabeças). Funciona como um radio button: "
        "marcar aqui desmarca automaticamente qualquer outra entrada HEAD. A primeira entrada HEAD que "
        "você adiciona já nasce marcada; qualquer HEAD adicionada depois nasce desmarcada"
    ),
    ...
    "rigger.prop.armature_rename_mode": "O que o 'Renomear Bones' renomeia",
    "rigger.prop.armature_rename_mode_item_all": (
        "Renomeia os bones originais -- todo bone gerado acompanha (L-Arm, L-Arm_CTRL, L-Arm_MCH...). Os "
        "nomes antigos ficam guardados: 'Use Original Names' no export .blockymodel/.blockyanim usa eles"
    ),
    "rigger.prop.armature_rename_mode_item_only_ctrl": (
        "Mantém os bones originais (ORG) e os MCH como estão; só os bones de controle (_CTRL, _IK, _Pole_CTRL, "
        "_Pole_Line) ganham os nomes do Hytale. Nada muda no modelo nem no export FBX"
    ),
    "rigger.prop.armature_rename_enabled": (
        "Mostra uma caixinha ao lado de cada picker de bone no Bone Settings e o botão 'Renomear Bones', "
        "que renomeia os bones marcados pros nomes padrão do Hytale (L-Arm, L-Forearm, Head, Pelvis...) "
        "pro espelhamento L/R funcionar. Os nomes antigos ficam guardados (botão de reverter, 'Use Original "
        "Names' no export)"
    ),
    "rigger.prop.ik_chain_rename_field": (
        "Renomeia este bone pro nome padrão do Hytale quando 'Renomear Bones' rodar. Desmarcado = mantém "
        "o nome original"
    ),
    "rigger.prop.ik_chain_rename_middle": (
        "Renomeia o Pole Reference e todo bone entre o Root e o Tip (Forearm, Forearm2... / Calf...) "
        "quando 'Renomear Bones' rodar. Desmarcado = mantêm os nomes originais"
    ),
    "rigger.tooltip.rename_bones": (
        "Renomeia os bones marcados no Bone Settings pros nomes padrão do Hytale. Remove o rig gerado antes, "
        "se houver (rode o Create Rig depois). Os nomes antigos ficam guardados pro botão de reverter e pro "
        "'Use Original Names' do export. Desfaça com Ctrl+Z"
    ),
    "rigger.tooltip.revert_bone_names": (
        "Reverte o 'Renomear Bones': todo bone renomeado volta pro nome original e os nomes de controle do "
        "Hytale (Only CTRL) são limpos -- os dois modos de uma vez. Remove o rig gerado antes, se houver (rode "
        "o Create Rig depois). Desfaça com Ctrl+Z"
    ),
    "panel.revert_nothing": "Nada pra reverter -- nenhum bone foi renomeado",
    "panel.revert_preview_title": "{count} nome(s) vão voltar pro original:",
    "exporter.prop.use_original_names": (
        "Bones renomeados com o 'Renomear Bones' (All Bones) são gravados com o nome que tinham antes do "
        "rename (o nome do jogo) em vez do novo. DESLIGADO (padrão): grava os nomes novos. Só disponível "
        "quando algum bone foi renomeado"
    ),
    "rigger.prop.ik_chain_root_count": (
        "Quantos bones de root (1-{max}). O Root 1 é o Origin principal; o Root 2 vira pai do Root 1, "
        "o Root 3 pai do Root 2, e assim por diante -- tudo que já está no Origin continua nele"
    ),
    "rigger.prop.ik_chain_root_slot_bone": (
        "Nome do bone ORIGINAL (não '_CTRL') usado como este root. Com 'New Bone' ligado, é o nome do "
        "bone a ser criado (vazio = 'Origin' pro Root 1, 'Root<N>' pros outros)"
    ),
    "rigger.prop.ik_chain_root_create": (
        "Cria este bone de root em vez de usar um do modelo. O bone criado existe só no Blender, como "
        "controle -- nunca é exportado pro .blockymodel/.blockyanim"
    ),
    ...
    "exporter.tooltip.blockymodel": (
        "Exporta a pose de repouso (bind pose) da Armature selecionada/ativa pro formato "
        ".blockymodel do Hytale -- posição/orientação vêm dos transforms atuais dos "
        "bones, shape/UV vêm do que foi capturado quando o personagem foi importado"
    ),
    "exporter.tooltip.fbx_model": (
        "Exporta a pose de repouso (bind pose) da Armature selecionada/ativa -- e, "
        "opcionalmente, suas malhas de referência -- pra um único arquivo .fbx. Nenhuma "
        "animação é incluída; use 'Hytale Animations (.fbx)' pra isso"
    ),
    "exporter.tooltip.fbx_anim": (
        "Exporta em lote uma ou mais Actions da Armature selecionada/ativa pra .fbx -- um "
        "arquivo por Action (rig + animação assada, e opcionalmente as malhas de "
        "referência), numa pasta escolhida"
    ),
    ...
    # -----------------------------------------------------------------
    # importer/bedrock.py -- IMPORT_OT_hytale_bedrock_model
    # -----------------------------------------------------------------
    "importer.tooltip.bedrock_model": (
        "Importa um modelo Bedrock do Minecraft (.json -- addons Bedrock e "
        "mods) convertido pro padrão do Hytale: "
        "escala, densidade de textura e inflate"
    ),
    "importer.bedrock_section_conversion": "Conversão",
    "importer.bedrock_geometry_identifier": "Geometria",
    "importer.bedrock_inflate_to_stretch": "Inflate para Stretch",
    "importer.bedrock_drop_locator_bones": "Remover Bones de Locator",
    "importer.prop.bedrock_model_format": (
        "O Minecraft não diz se o modelo é criatura ou bloco, então "
        "escolha aqui. O Minecraft usa 16 unidades por bloco; o Hytale "
        "usa 64 (Character) ou 32 (Prop). O modelo mantém o tamanho no "
        "mundo -- geometria, UV e textura são ampliadas pra combinar"
    ),
    "importer.prop.bedrock_model_format_item_character": (
        "Criaturas, mobs, personagens, vestíveis -- geometria e textura x4 (16 -> 64 por bloco)"
    ),
    "importer.prop.bedrock_model_format_item_prop": (
        "Blocos, móveis, props -- geometria e textura x2 (16 -> 32 por bloco)"
    ),
    "importer.prop.bedrock_geometry_identifier": (
        "Qual geometria importar quando o arquivo tem mais de uma (ex: "
        "geometry.meu_modelo ou só meu_modelo). Vazio = a primeira"
    ),
    "importer.prop.bedrock_armature_name": (
        "Nome pra Armature nova e sua collection. Vazio = nome da "
        "geometria (sem 'geometry.')"
    ),
    "importer.prop.bedrock_inflate_to_stretch": (
        "O Hytale não tem 'inflate'. Converte pra 'stretch' em torno do "
        "centro do cubo, que dá o mesmo resultado sem mexer na UV. "
        "Cubos achatados (tamanho 0) continuam achatados"
    ),
    "importer.prop.bedrock_drop_locator_bones": (
        "Pula bones que só guardam locators (pontos de efeito/item usados "
        "por mods do Minecraft, ex: locator_mouth) e não têm cubos"
    ),
    "importer.prop.bedrock_texture_mode": (
        "'Automático' procura o PNG do mesmo jeito que o importador de "
        ".blockymodel: um PNG na pasta do modelo cujo nome começa com o nome "
        "do modelo (nome do arquivo sem '.geo.json'), 'Texture.png', ou uma "
        "subpasta '<NomeDoModelo>_Textures' -- e, como último recurso, uma "
        "pasta com 'texture' no nome um nível acima do modelo. Variantes são "
        "todas carregadas e empilhadas no material. 'Manual' usa só o "
        "arquivo que você escolher"
    ),
    "importer.prop.bedrock_texture_mode_item_auto": (
        "Detecta o PNG automaticamente (mesma busca do importador de .blockymodel)"
    ),
    "importer.prop.bedrock_texture_filepath": (
        "O PNG do modelo (Texture Mode = Manual) -- ou uma PASTA: aí todo PNG "
        "dela e das subpastas cujo nome começa com o nome do modelo (ou com o "
        "nome sem o último '_pedaço') é carregado, mesmo filtro do "
        "Automático; o primeiro fica ligado, os outros empilhados no "
        "material. Modelos Bedrock não guardam a textura. A cópia importada "
        "é ampliada (sem borrar) pra densidade do Hytale; os arquivos "
        "originais não são alterados"
    ),

    # -----------------------------------------------------------------
    # importer/common_options.py -- opções COMPARTILHADAS entre os
    # importadores (texto igual pra todos os formatos)
    # -----------------------------------------------------------------
    "importer.prop.unit_scale": (
        "Unidades do Blender por unidade do jogo, pra modelos Character (64 "
        "unidades por bloco) -- o padrão 1/64 faz 1 bloco = 1 metro. Modelos "
        "Prop (32 por bloco) usam o dobro automaticamente, pra os dois "
        "ficarem no mesmo tamanho no Blender. Os exportadores usam o mesmo "
        "valor pra converter de volta, então deixe no padrão a menos que "
        "tenha certeza"
    ),
    "importer.prop.generate_reference_boxes": (
        "Cria uma malha pra cada cubo do modelo, parentada à Armature e com "
        "skin (peso 100%) no seu bone via Vertex Group + modificador Armature"
    ),
    "importer.prop.generate_uvs": (
        "Gera as coordenadas UV das malhas de referência a partir do "
        "retângulo em pixels de cada face do modelo"
    ),
    "importer.prop.create_material": (
        "Cria um material com a textura encontrada pelo 'Texture Mode', "
        "ligado no Base Color/Alpha pelas UVs geradas"
    ),
}
