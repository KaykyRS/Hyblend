...
from bpy.types import Armature, Object, WindowManager
...
from .helpers import control_name, find_org_path, resolve_pelvis_org_name, source_org_of  # noqa: F401 (reexport pro anim/)
from .constraints import switch_property_name
...
from .validate import RIG_MT_hytale_clear_generated_menu, RIG_OT_hytale_clear_generated, RIG_OT_hytale_validate_rig
from .shape_edit import (
    ...
    register_shape_edit_self_heal,
    unregister_shape_edit_border,
    unregister_shape_edit_self_heal,
)
from .texture_picker import RIG_OT_hytale_texture_picker_create, RIG_OT_hytale_texture_picker_remove
from .camera import RIG_OT_hytale_camera_create, RIG_OT_hytale_camera_remove
from .generate import RIG_OT_hytale_generate_rig
from .rename import RIG_OT_hytale_rename_bones, RIG_OT_hytale_revert_bone_names
...
_CLASSES = (
    ...
    RIG_OT_hytale_shape_edit_mode_enter,
    RIG_OT_hytale_shape_edit_mode_finish,
    # v0.17 -- atalho de recuperação quando o objeto ativo mudou
    # enquanto uma Armature ficou em Shape Edit Mode (ver
    ...
    # Parte 4 -- Rename Bones (padrão de nomes do Hytale). Sem dependência
    # de ordem conhecida com o resto -- acrescentado no fim.
    RIG_OT_hytale_rename_bones,
    RIG_OT_hytale_revert_bone_names,
)
...
def _assign_dynamic_properties(lang=None):
    ...
    # Parte 4 -- modo do Rename Bones. Ordem dos itens FIXA: o .blend
    # guarda o Enum pelo índice (item novo sempre no fim).
    Armature.hytale_rename_mode = EnumProperty(
        name="Rename Mode",
        items=[
            ("ALL_BONES", "All Bones", tr("rigger.prop.armature_rename_mode_item_all", lang)),
            ("ONLY_CTRL", "Only CTRL", tr("rigger.prop.armature_rename_mode_item_only_ctrl", lang)),
        ],
        default="ALL_BONES",
        description=tr("rigger.prop.armature_rename_mode", lang),
    )
    Armature.hytale_rename_enabled = BoolProperty(
        name="Rename Bones",
        description=tr("rigger.prop.armature_rename_enabled", lang),
        default=False,
    )
...
def register():
    ...
    # v0.17 -- referência direta ao Armature dono, gravada na malha do
    # widget durante o Vertex Edit Mode (ver
    # RIG_OT_hytale_shape_vertex_edit_mode_enter/shape_edit.py).
    # PointerProperty em vez de custom property de STRING com o nome
    # -- sobrevive a um rename do Armature no meio da edição (bug real
    # já corrigido, ver docstring da seção "Vertex Edit Mode" em
    # shape_edit.py). Sem description= própria (não aparece em nenhum
    # painel), não precisa entrar em _assign_dynamic_properties.
    Object.hytale_vertex_edit_armature_ptr = PointerProperty(type=Object)
    ...
    # v0.17 -- rede de segurança contra estado órfão de Shape/Vertex
    # Edit Mode (ver _self_heal_shape_edit_state/shape_edit.py) --
    # roda em load_post/undo_post/redo_post, e uma vez já aqui no
    # register() (cobre um arquivo aberto ANTES do addon ligar).
    register_shape_edit_self_heal()
...
def unregister():
    ...
    del Object.hytale_vertex_edit_armature_ptr
    ...
    del Armature.hytale_rename_enabled
    del Armature.hytale_rename_mode
